...
//		  std::cout << reponse_data << std::endl;
		  ...
		  else
		  {
			  ...
			  cout<<"Status = "<<decode_val.at("Status")<<endl;
			  cout<<"Result = "<<decode_val.at("Result")<<endl;

			  if ("0"!=decode_val.at("Status").get_string())
				  cout<<"FailReason : "<<decode_val.at("FailReason")<<endl;
			  else
			  {
				 cout<<"ad="<<decode_val.at("NetTopologicalInfo").at("gw_info").at("pppoe")<<endl;
				 cout<<"mac="<<decode_val.at("NetTopologicalInfo").at("gw_info").at("mac")<<endl;
				  if( auto p = decode_val.at("NetTopologicalInfo").at("topo_info").if_array())
				  {

		//			  cout<<"decode_val[row]="<<decode_val.at("rows")<<endl;
					    cout<<"device numbers :"<< p->size()<<endl;
